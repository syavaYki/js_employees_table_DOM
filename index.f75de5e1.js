...
function sortRows(rows, sortIndex, accending) {
    rows.sort((a, b)=>{
        const aClean = a.cells[sortIndex].textContent?.replace("$", "").replace(",", "");
        const bClean = b.cells[sortIndex].textContent.replace("$", "").replace(",", "");
        if (isNaN(aClean)) {
            if (accending) return a.cells[sortIndex].textContent?.localeCompare(b.cells[sortIndex].textContent);
            return b.cells[sortIndex].textContent?.localeCompare(a.cells[sortIndex].textContent);
        } else {
...
function validator(data, formData = true) {
    ...
    if (formData || !formData && data.get("office")) {
        if (!offices.includes(data.get("office"))) {
            pushNotification("Error", `Invalid office locatioin please select from following: ${offices.join(", ")}`, "error");
            return false;
        }
    }
...
for(let i = 0; i < tHead?.rows[0].cells.length; i++)sortMapIndx[headerCells[i].textContent] = i;
tHead?.addEventListener("click", (e)=>{
    ...
    for(let row = 0; row < tableRows.length; row++)document.querySelector("tbody")?.append(tableRows[row]);
});
tBody?.addEventListener("click", (e)=>{
    for(let i = 0; i < document.querySelector("tbody").rows.length; i++)document.querySelector("tbody").rows[i].classList.remove("active");
...
tBody?.addEventListener("dblclick", (e)=>{
    function recInput(ev) {
        ...
        const keyMap = tHead.rows[0].cells[[
            ...activeCell.parentElement.cells
        ].indexOf(activeCell)].textContent?.toLocaleLowerCase();
